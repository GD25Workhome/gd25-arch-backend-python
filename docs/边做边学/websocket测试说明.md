...
服务启动后，WebSocket 端点地址为：`ws://localhost:8100/ws/{user_id}`
...
如果遇到以下错误：
```
Refused to connect to 'ws://localhost:8100/ws/test_user' because it violates 
...
# 或使用 npx（无需安装）
npx wscat -c ws://localhost:8100/ws/test_user
```
...
```bash
# 连接到 WebSocket
wscat -c ws://localhost:8100/ws/test_user
...
async def test_websocket(user_id: str = "test_user"):
    ...
    Args:
        user_id: 用户 ID
    """
    uri = f"ws://localhost:8100/ws/{user_id}"
...
**解决方法：**
1. 确认服务已启动：`curl http://localhost:8100/health`
2. 检查端口是否正确（默认 8100）
3. 检查防火墙设置
...
1. **使用 API 接口触发**（需要实现）：
   ```bash
   curl -X POST http://localhost:8100/api/broadcast \
