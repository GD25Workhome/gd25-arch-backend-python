# FastAPI 后端脚手架 - 快速开始指南

本指南将帮助你在 10 分钟内快速启动一个新项目。

## 一、前置要求

- Python 3.10+
- PostgreSQL 14+ 或 MySQL 8.0+（用于生产环境）
- Git
- 基本的 Python 和 FastAPI 知识

## 二、快速开始（5 步）

### 步骤 1：克隆脚手架（1 分钟）

```bash
# 克隆脚手架仓库
git clone https://github.com/your-org/gd25-arch-backend-python my-project

# 进入项目目录
cd my-project

# 删除原有的 Git 历史，重新初始化
rm -rf .git
git init
```

### 步骤 2：配置环境（2 分钟）

```bash
# 创建虚拟环境（推荐使用 conda）
conda create -n my-project python=3.10
conda activate my-project

# 安装依赖
pip install -r requirements.txt

# 创建环境变量文件
# 注意：如果 env.example 不存在，需要手动创建 .env 文件
# 至少需要配置 DATABASE_URL
```

创建 `.env` 文件：

```bash
# .env
APP_NAME=my-project
APP_VERSION=1.0.0
ENVIRONMENT=development
DEBUG=true

# 数据库配置（必需）
DATABASE_URL=postgresql://user:password@localhost:5432/mydb
# 或 MySQL:
# DATABASE_URL=mysql+pymysql://user:password@localhost:3306/mydb

# 日志配置
LOG_LEVEL=INFO
LOG_FORMAT=json
```

### 步骤 3：初始化数据库（1 分钟）

```bash
# 方式 1：使用数据库初始化脚本（推荐）
# 自动创建数据库（如果不存在）
python scripts/init_db.py

# 如果需要安装 PostgreSQL pgvector 扩展（仅 PostgreSQL）
python scripts/init_db.py --install-pgvector

# 方式 2：手动创建数据库
# PostgreSQL:
# psql -U postgres
# CREATE DATABASE mydb;

# MySQL:
# mysql -u root -p
# CREATE DATABASE mydb;
```

```bash
# 创建数据库迁移脚本
alembic revision --autogenerate -m "Initial migration"

# 执行数据库迁移
alembic upgrade head
```

### 步骤 4：启动服务（30 秒）

```bash
# 启动开发服务器
uvicorn app.main:app --reload

# 或使用启动脚本
./scripts/start_dev.sh
```

### 步骤 5：验证运行（30 秒）

```bash
# 测试健康检查接口
curl http://localhost:8090/health

# 访问 API 文档
# 浏览器打开：http://localhost:8090/docs
```

## 三、下一步：使用示例代码

脚手架提供了完整的用户管理示例代码，你可以：

1. **查看示例代码**：
   - Model: `app/models/user.py`
   - Schema: `app/schemas/user.py`
   - Repository: `app/repositories/user_repository.py`
   - Service: `app/services/user_service.py`
   - API: `app/api/users.py`

2. **注册示例路由**（在 `app/main.py` 中）：
   ```python
   from app.api.users import router as users_router
   app.include_router(users_router, prefix="/api/v1")
   ```

3. **测试示例 API**：
   - 访问 http://localhost:8090/docs
   - 尝试创建、查询、更新、删除用户

详细说明请参考：[示例代码使用说明](./示例代码使用说明.md)

## 四、开发你的第一个功能

### 4.1 创建模型

```python
# app/models/product.py
from app.db.base import BaseModel
from sqlalchemy import Column, String, Float

class Product(BaseModel):
    __tablename__ = "products"
    
    name = Column(String(100), nullable=False)
    price = Column(Float, nullable=False)
```

### 4.2 创建 Schema

```python
# app/schemas/product.py
from pydantic import BaseModel

class ProductCreate(BaseModel):
    name: str
    price: float

class ProductResponse(BaseModel):
    id: int
    name: str
    price: float
```

### 4.3 创建 Repository

```python
# app/repositories/product_repository.py
from app.repositories.base import BaseRepository
from app.models.product import Product

class ProductRepository(BaseRepository[Product]):
    def __init__(self, db: Session):
        super().__init__(Product, db)
```

### 4.4 创建 Service

```python
# app/services/product_service.py
from app.repositories.product_repository import ProductRepository
from app.schemas.product import ProductCreate

class ProductService:
    def __init__(self, db: Session):
        self.repository = ProductRepository(db)
    
    def create_product(self, product_data: ProductCreate):
        return self.repository.create(product_data.model_dump())
```

### 4.5 创建 API

```python
# app/api/products.py
from fastapi import APIRouter, Depends
from app.services.product_service import ProductService
from app.dependencies import get_db

router = APIRouter(prefix="/products", tags=["产品"])

@router.post("/")
async def create_product(
    product_data: ProductCreate,
    db: Session = Depends(get_db)
):
    service = ProductService(db)
    product = service.create_product(product_data)
    return product
```

### 4.6 注册路由

在 `app/main.py` 中：

```python
from app.api.products import router as products_router
app.include_router(products_router, prefix="/api/v1")
```

### 4.7 数据库迁移

```bash
alembic revision --autogenerate -m "Add product model"
alembic upgrade head
```

## 五、常用命令

### 开发

```bash
# 启动开发服务器
uvicorn app.main:app --reload

# 运行测试
pytest

# 运行测试并生成覆盖率报告
pytest --cov=app --cov-report=html
```

### 数据库

```bash
# 创建迁移脚本
alembic revision --autogenerate -m "描述"

# 执行迁移
alembic upgrade head

# 回滚迁移
alembic downgrade -1

# 查看迁移历史
alembic history
```

### 代码质量

```bash
# 格式化代码（如果安装了 black）
black app/

# 检查代码风格（如果安装了 flake8）
flake8 app/

# 类型检查（如果安装了 mypy）
mypy app/
```

## 六、项目结构说明

```
my-project/
├── app/                    # 应用主目录
│   ├── api/                # API 路由
│   │   ├── users.py        # 用户 API（示例）
│   │   └── __init__.py
│   ├── models/             # 数据模型
│   │   ├── user.py        # 用户模型（示例）
│   │   └── __init__.py
│   ├── schemas/            # Pydantic Schema
│   │   ├── user.py        # 用户 Schema（示例）
│   │   └── __init__.py
│   ├── repositories/       # 数据访问层
│   │   ├── base.py        # Repository 基类
│   │   ├── user_repository.py  # 用户 Repository（示例）
│   │   └── __init__.py
│   ├── services/           # 业务逻辑层
│   │   ├── user_service.py  # 用户服务（示例）
│   │   └── __init__.py
│   ├── db/                 # 数据库相关
│   ├── utils/              # 工具类
│   ├── config.py           # 配置管理
│   └── main.py             # 应用入口
├── tests/                  # 测试文件
├── alembic/                # 数据库迁移
├── docs/                    # 文档
├── requirements.txt        # 依赖
└── .env                    # 环境变量（不提交到 Git）
```

## 七、常见问题

### Q1: 数据库连接失败？

**A**: 
1. 检查 `.env` 文件中的 `DATABASE_URL` 是否正确
2. 确保数据库服务已启动
3. 如果数据库不存在，运行 `python scripts/init_db.py` 自动创建数据库
4. 确保数据库用户有足够的权限（创建数据库、创建表等）

### Q1.1: 如何初始化数据库？

**A**: 使用数据库初始化脚本：
```bash
# 创建数据库（如果不存在）
python scripts/init_db.py

# 创建数据库并安装 pgvector 扩展（仅 PostgreSQL）
python scripts/init_db.py --install-pgvector
```

详细说明请参考：[数据库初始化使用说明](./数据库初始化使用说明.md)

### Q2: 如何添加新的配置项？

**A**: 继承 `Settings` 类添加自定义配置项，参考 `app/config.py` 中的注释。

### Q3: 如何添加认证？

**A**: 在 `app/dependencies.py` 中添加认证依赖，参考文档中的扩展说明。

### Q4: 如何运行测试？

**A**: 运行 `pytest`，测试使用 SQLite 内存数据库，无需配置实际数据库。

### Q5: 如何部署到生产环境？

**A**: 
1. 设置 `ENVIRONMENT=production`
2. 配置生产数据库
3. 使用 `gunicorn` 或 `uvicorn` 部署
4. 参考 `scripts/start_prod.sh`

## 八、参考资源

- [项目 README](../README.md)
- [示例代码使用说明](./示例代码使用说明.md)
- [Repository 使用示例](./repository使用示例.md)
- [启动方式说明](./启动方式说明.md)
- [测试执行说明](./测试执行说明.md)
- [FastAPI 官方文档](https://fastapi.tiangolo.com/)

## 九、获取帮助

- 查看项目文档：`docs/` 目录
- 查看示例代码：`app/models/`, `app/api/` 等目录
- 提交 Issue：GitHub Issues
- 查看开发计划：`../开发计划.md`

---

**祝你开发愉快！** 🚀

